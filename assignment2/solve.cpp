...
double * IsingMonteCarlo::solve2D(int calibration, int MC, int N_bins, int rank){
  ...
  */
  ...
  for (int temp = 0; temp < m_nT; temp++){
    ...
    varianceM = exp_val_M2 - exp_values[0]*exp_values[0];
    m_xi = varianceM/((double) m_T[temp]);
    exp_values[2] += m_xi;
    exp_values[3] += varianceM;
    }

    // intrinsic adjust and total mean
    exp_values[2] = 1./((double) m_L2)*exp_values[2];
    exp_values[3] = 1./((double) m_L2)*exp_values[3];
    // Scaling by L^2
    for(int i= 0; i < n_expv ;i++){
         exp_values[i] = ((double) 1/(m_L2*N_bins))*exp_values[i];
    }

    // L^4 for variance
    //varianceM = ((double) 1/(m_L2*m_L2*N_bins))*varianceM;
